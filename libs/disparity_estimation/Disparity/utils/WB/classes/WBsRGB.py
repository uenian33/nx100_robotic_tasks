...
class WBsRGB:
  def __init__(self, gamut_mapping=2, upgraded=0, weight_pth='None'):
    if upgraded == 1:
      self.features = np.load(weight_pth+'features+.npy')  # encoded features
      self.mappingFuncs = np.load(weight_pth+'mappingFuncs+.npy')  # correct funcs
      self.encoderWeights = np.load(weight_pth+'encoderWeights+.npy')  # PCA matrix
      self.encoderBias = np.load(weight_pth+'encoderBias+.npy')  # PCA bias
      self.K = 75  # K value for NN searching
    else:
      self.features = np.load(weight_pth+'features.npy')  # encoded features
      self.mappingFuncs = np.load(weight_pth+'mappingFuncs.npy')  # correction funcs
      self.encoderWeights = np.load(weight_pth+'encoderWeights.npy')  # PCA matrix
      self.encoderBias = np.load(weight_pth+'encoderBias.npy')  # PCA bias
